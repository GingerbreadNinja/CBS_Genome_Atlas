...
def register_job(accession, version, maketag):
    job_id = 0
...
def process_one_genome(accession, version, maketag):
    ...
    job_uuid = register_job(accession, version, maketag)

    # TODO get this information from a config file
    logging_dir = "/home/panfs/cbs/projects/cge/data/public/genome_sync/log/"
    call(["xmsub", "-l", "mem=4gb,walltime=3600,procs=8,partition=cge-cluster", "-de", "-ro", logging_dir + accession + "_" + job_uuid + ".log", "-re", logging_dir + accession + "_" + job_uuid + ".out", "-N", accession + "_" + str(version) + "_" + job_uuid, "-r", "y", "make -i -k", maketag, "ACCESSION=" + accession, "VERSION=" + str(version), "JOB_UUID=" + job_uuid])
...
def process_new_genomes(backfill, maketag):
    ...
    for accession, version in new_genomes(last_runtime):
        ...
        process_one_genome(accession, version, maketag)
    
def failed_genomes():
    # get list of all genomes to reprocess;
    return []

def process_failed_genomes(maketag):
    for accession in failed_genomes():
        process_one_genome(accession, version, maketag)

def main(argv):
    try:
        opts, args = getopt.getopt(sys.argv[1:], "h", ["help", "latest", "accession=", "version=", "av=", "backfill", "maketag="])
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            usage()
    if plan == "all":
        process_new_genomes(backfill, maketag)
        #process_failed_genomes(maketag) TODO
    elif plan == "one":
